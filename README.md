...
- 📂 **智能操作** — 根据内容类型自动提供专属操作：图片→预览/保存、文件→资源管理器打开、链接→浏览器打开、代码→格式高亮，一键直达
- 😀 **Emoji 表情面板** — 内置完整 Emoji 库，单击复制、双击直接粘贴，支持搜索和肤色切换（可在设置中开关）
...
|--------|------|
| `Alt` + `` ` `` | 呼出 / 隐藏面板 |
| `Alt` + `1` ~ `6` (Windows) / `Cmd` + `1` ~ `6` (Mac) | 切换分类 tab |
| `Alt` + `Space` (Windows) | 预览选中条目（兜底） |

**应用内快捷键（面板聚焦时生效）**

| 快捷键 | 动作 |
|--------|------|
| `Tab` / `Shift`+`Tab` | 切换内容类型（主视图） / 切换 Emoji 分类（Emoji 视图） |
| `←` / `→` | 切换内容类型 |
| `↑` / `↓` | 上下选择条目 |
| `Enter` | 粘贴选中条目 |
| `Space`（单击） | 预览 / 查看详情 |
| `Space`（双击） | 专属操作（图片→保存 / 文件→资源管理器显示 / 链接→浏览器打开） |
| `Ctrl` + 点击 (Windows) / `Cmd` + 点击 (Mac) | 切换选中状态（多选、反选） |
| `Ctrl` + 右键 (Windows) / `Cmd` + 右键 (Mac) | 追加选择（右键菜单） |
| `Esc` | 关闭面板 / 清除多选 / 退出设置或 Emoji 视图 |
| `Delete` / `Backspace` | 删除选中条目（支持批量） |
| 双击条目 | 粘贴（当粘贴触发模式设为"双击"时） |
...
<details>
<summary>Emoji选择器</summary>

![Emoji选择器](./assets/images/gopaste_emoji.png)
